...
// Complete MD5 implementation (same as in the main handler)
...
// PayFast signature calculation logic (same as in main handler)
function calculatePayFastSignature(rawBody: string, passphrase: string): string {
  console.log('=== PayFast Signature Calculation Test ===');
  ...
  // Parse raw body to preserve encoding
  ...
  for (const pair of pairs) {
    ...
    // Skip empty values (check both raw and decoded)
    if (value === '' || decodeURIComponent(value).trim() === '') {
      console.log(`  Excluding: ${key} (empty value)`);
      continue;
    }
...
serve(async (req) => {
  try {
    ...
    // Test cases with known PayFast signature scenarios
    const testCases = [
      {
        name: 'Simple Test Case',
        rawBody: 'm_payment_id=123&pf_payment_id=456&payment_status=COMPLETE&item_name=Test%20Item&amount_gross=10.00&merchant_id=10000100&signature=dummy',
        passphrase: 'testpassphrase',
        description: 'Basic test with simple parameters'
      },
      {
        name: 'Real PayFast ITN Example (without signature)',
        rawBody: 'm_payment_id=c9f3dea7-cb8e-4f3a-9200-f6ed2fc0d7a0&pf_payment_id=2591915&payment_status=COMPLETE&item_name=Captain%27s+Club+Subscription&item_description=First+payment+for+Captain%27s+Club+Subscription&amount_gross=99.00&amount_fee=-2.30&amount_net=96.70&custom_str1=f50ff0ee-0a56-4c24-bd2f-d46761644db3&custom_str2=&custom_str3=&custom_str4=&custom_str5=&custom_int1=&custom_int2=&custom_int3=&custom_int4=&custom_int5=&name_first=lk&name_last=&email_address=s%40s.com&merchant_id=10039481&token=efff70cc-f23c-42c6-be63-80a3a4b9a12c&billing_date=2025-06-28',
        passphrase: 'sandboxAlphaBravo2024',
        description: 'Real PayFast ITN data (sandbox)'
      },
      {
        name: 'Test with Empty Fields',
        rawBody: 'm_payment_id=123&pf_payment_id=456&payment_status=COMPLETE&item_name=Test&custom_str1=&custom_str2=value&amount_gross=10.00&merchant_id=10000100',
        passphrase: 'test',
        description: 'Test handling of empty fields'
      }
    ];
    ...
    for (const testCase of testCases) {
      ...
      try {
        const calculatedSignature = calculatePayFastSignature(testCase.rawBody, testCase.passphrase);
    ...
    // Additional verification: Test the exact signature string building logic
    console.log('\n\n=== SIGNATURE STRING BUILDING VERIFICATION ===');
    const testString = 'merchant_id=10000100&m_payment_id=123&pf_payment_id=456&payment_status=COMPLETE&item_name=Test&amount_gross=10.00&passphrase=test';
    const expectedMD5 = md5(testString);
    console.log(`Test string: ${testString}`);
    console.log(`MD5 result: ${expectedMD5}`);
    
    return new Response(JSON.stringify({
      message: 'PayFast signature calculation test completed',
      testResults: results,
      verificationTest: {
        testString: testString,
        md5Result: expectedMD5
      },
      summary: {
    ...
    console.error('Error in PayFast signature test:', error);
    return new Response(JSON.stringify({
      error: error.message,
      stack: error.stack
    }), {
